...
const fetchRepoData = async (name?: string) => {
  ...
  const dataRequest = await fetch(
    `https://api.github.com/repos/piyushyadav0191/${name}/releases/latest`,
    authHeaders,
  );
  ...
  const extraRepoData = {
    success: false,
    url: `https://github.com/piyushyadav0191/${name}`,
  };
  const defaultDownloadLink = `https://github.com/piyushyadav0191/${name}/releases/latest/`;
